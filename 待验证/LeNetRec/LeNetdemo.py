...
# root_dir = "train_character2"
# labels_train = open("idx.txt")
# test_dir = "train_character2"
# labels_test = open("idx.txt")
#
# #
# # 数据集集成打包+ 数据集批量化。
#
...
# 定义损失函数和优化器
loss_function = torch.nn.CrossEntropyLoss()   # 交叉熵
optimizer = torch.optim.SGD(  # SGD优化器
    model.parameters(),
    lr=0.001,
    momentum=0.9
)
...
def change_img(data):  # 定义一个数据类型转换子类。
...
def demo(pre_img):  # 预测任务
    ...
    model.eval()  # 测试模式
    tensor = change_img(pre_img)  # 将图片转化成tensor
